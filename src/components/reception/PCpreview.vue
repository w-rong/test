  ...
  <div class="report_content" id="report_download">
    ...
    <div class="report_content_all" >
      ...
      <div class="report_resources report_resources_margin">
        ...
        <div class="report_resources_content">
          普陀最热门的旅游景区是{{scenicName[scenicName.length-1]}}景区，接待游客
          <span
            class="import_account"
          >{{totalTourist}}</span>
          人次,
          其次是{{scenicName[scenicName.length-2]}},接待游客
          <span
            class="import_account"
          >{{ptTourist}}</span>人次。
        </div>
  ...
  methods: {
    async getSpringData() {
      ...
      for (var key in data) {
        ...
        if (key == "portraitConsume") {
          for (var key1 in data[key]) {
            ...
            if (key1 == "marriedRatio") {
              // var haveMarr = JSON.parse(data[key][key1])["married"];
              var haveMarr = data[key][key1]["married"];
              this.marriedData = Number(haveMarr * 100).toFixed(2);
            }
            if (key1 == "childRatio") {
              // var haveMarr = JSON.parse(data[key][key1])["has"];
              var haveMarr = data[key][key1]["has"];
              this.childData = Number(haveMarr * 100).toFixed(2);
            }
            if (key1 == "travelDrRatio") {
              // var haveMarr = JSON.parse(data[key][key1])["expert"];
              var haveMarr = data[key][key1]["expert"];
              this.childData = Number(haveMarr * 100).toFixed(2);
            }
            if (key1 == "sexRatio") {
              // var haveMarr = JSON.parse(data[key][key1])["female"];
              var haveMarr = data[key][key1]["female"];
              this.female = Number(haveMarr * 100).toFixed(2);
              // var haveMarr1 = JSON.parse(data[key][key1])["male"];
              var haveMarr1 = data[key][key1]["male"];
              this.male = Number(haveMarr1 * 100).toFixed(2);
            }
        ...
        // 常驻市
        if (key == "city") {
          // var objData1 = jsonSort(JSON.parse(data[key]), false).slice(0, 6);
    ...
    downLoad() {
      // document.querySelector('#report_download')
      // document.getElementById("report_download")
      html2canvas(document.getElementById("report_download"), {
        dpi: 200, //导出pdf清晰度
