<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入乡村名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">乡村名称</td>
              <td style="width:20%">乡村地址</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              ...
              <td>{{item.lat}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.villageId	)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.villageId	)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              ...
              <el-col :span="22">
                <el-col :span="12">
                <el-form-item label="乡村地址" prop="address">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseVillage"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        address: [
          { required: true, message: "请输入乡村地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择乡村等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择乡村", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入乡村最大承载量", trigger: "blur" }
        ]
      },
      token: {}
    };
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseVillage'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseVillage?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseVillage`,
        qs.stringify({
          villageName: this.ruleForm.name,
          address: this.ruleForm.address,
          lat: this.ruleForm.lat,
          lng: this.ruleForm.lng,
          tel: this.ruleForm.leaderPhone,
          scenicDetails: this.ruleForm.introduce,
          villagePic: this.ruleForm.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseVillage?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.villageName;
        ...
        this.ruleFormChange.introduce = this.editScenicList.scenicDetails;
        this.ruleFormChange.imgUrl = this.editScenicList.villagePic;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseVillage`,
        qs.stringify({
          villageId	:this.editScenicList.villageId,
          villageName: this.ruleFormChange.name,
          ...
          scenicDetails: this.ruleFormChange.introduce,
          villagePic: this.ruleFormChange.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseVillage?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
