    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入景区村名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">景区村名称</td>
              <td style="width:10%">景区村地址</td>
              <td style="width:10%">景区村等级</td>
            ...
            <tr>
              <td v-show="scenicList.length == ''" colspan="10" class="noinfo">没有找到匹配的记录</td>
            </tr>
            <tr v-for="(item, index) in scenicList" :key="index">
              ...
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.scenicVillageId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.scenicVillageId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="景区村名称" prop="name">
                    <el-input v-model="ruleForm.name"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="景区村等级" prop="region">
                    <el-select v-model="ruleForm.region" placeholder="请选择景区村等级">
              ...
              <el-col :span="22">
                <el-col :span="12">
                <el-form-item label="景区村地址" prop="address">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                ...
                <el-col :span="12">
                  <el-form-item label="景区村等级" prop="region">
                    <el-select v-model="ruleFormChange.region" placeholder="请选择景区村等级">
                      <el-option label="未评级" value="0"></el-option>
                      <el-option label="A" value="1"></el-option>
                      <el-option label="AA" value="2"></el-option>
                      <el-option label="AAA" value="3"></el-option>
...
<el-col :span="10" style="margin-left:10px;margin-top:5px">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importPtScenicVillage"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入景区村名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入景区村地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择景区村等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择景区村", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入景区村最大承载量", trigger: "blur" }
        ]
      },
    };
  },
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelPtScenicVillage'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listPtScenicVillage?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getPtScenicVillage?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async editScenicLists() {
      console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updatePtScenicVillage`,
        qs.stringify({
          scenicVillageId:this.editScenicList.scenicVillageId,
          ...
          level: this.ruleFormChange.region,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deletePtScenicVillage?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
