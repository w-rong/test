<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入特产名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">特产名称</td>
              <td style="width:20%">特产内容</td>
              <td style="width:10%">商场名称</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              ...
              <td>{{item.specialtyContent}}</td>
              <td>{{item.shoppingName}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.specialtyId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.specialtyId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseSpecialty"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入特产名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入特产地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择特产等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择特产", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入特产最大承载量", trigger: "blur" }
        ]
      },
      token: {}
    };
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseSpecialty'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseSpecialty?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseSpecialty`,
        qs.stringify({
          specialtyName: this.ruleForm.name,
          shoppingName: this.ruleForm.address,
          specialtyContent: this.ruleForm.lat,
          specialtyPic: this.ruleForm.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseSpecialty?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.specialtyName;
        this.ruleFormChange.address = this.editScenicList.shoppingName;
        this.ruleFormChange.lat = this.editScenicList.specialtyContent;
        this.ruleFormChange.imgUrl = this.editScenicList.specialtyPic;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseSpecialty`,
        qs.stringify({
          specialtyId:this.editScenicList.specialtyId,
          specialtyName: this.ruleFormChange.name,
          shoppingName: this.ruleFormChange.address,
          specialtyContent: this.ruleFormChange.lat,
          specialtyPic: this.ruleFormChange.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseSpecialty?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
