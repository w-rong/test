    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入人文设施名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">人文设施名称</td>
              <td style="width:20%">人文设施地址</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              <td>{{item.humanityName}}</td>
              <td>{{item.address}}</td>
              <td>{{item.leader}}</td>
              <td>{{item.lng}}</td>
              <td>{{item.lat}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.humanityId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.humanityId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="人文设施名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                <el-form-item label="人文设施地址" prop="address">
...
<el-col :span="8" style="margin-left:10px;margin-top:5px">
              ...
              <el-col :span="22">
                <el-form-item label="文化市场综合执法机构介绍">
...
<el-col :span="8" style="margin-left:10px;margin-top:5px">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseHumanity"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入人文设施名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入人文设施地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择人文设施等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择人文设施", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入人文设施最大承载量", trigger: "blur" }
        ]
      },
      token: {}
    };
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseHumanity'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseHumanity?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseHumanity`,
        qs.stringify({
          humanityName: this.ruleForm.name,
          address: this.ruleForm.address,
          lat: this.ruleForm.lat,
          lng: this.ruleForm.lng,
          leader: this.ruleForm.leaderPhone,
          remark:this.ruleForm.introduce,
          humanityPic:this.ruleForm.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseHumanity?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.humanityName;
        ...
        this.ruleFormChange.leaderPhone = this.editScenicList.leader;
        this.ruleFormChange.introduce = this.editScenicList.remark;
        this.ruleFormChange.imgUrl = this.editScenicList.humanityPic;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseHumanity`,
        qs.stringify({
          humanityId:this.editScenicList.humanityId,
          humanityName: this.ruleFormChange.name,
          address: this.ruleFormChange.address,
          lat: this.ruleFormChange.lat,
          lng: this.ruleFormChange.lng,
          leader: this.ruleFormChange.leaderPhone,
          remark: this.ruleFormChange.introduce,
          humanityPic: this.ruleFormChange.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseHumanity?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
