<template>
  <div class="basis_arg_all_content">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/setPtFormHouse"
        multiple
        name="uploadFile"
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelPtFormHouseVO";
      // let res = await this.$http.get(`/excel/getExcelPtFormHouseVO`);
      // console.log(res);
    },
    beforeUpload(file) {
      // console.log(file);
