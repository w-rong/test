<template>
  <div class="basis_hotel_all_content">
    <Navbar></Navbar>
    <div class="basis_hotel_content">
      <div class="basis_hotel_content_right_content">
        <div class="basis_hotel_content_right_content_title">
          <input type="text" placeholder="请输入宾馆名称" v-model="unitName" />
        ...
        <div class="basis_hotel_content_right_content_table">
          <table>
            <tr class="basis_hotel_content_right_content_table_title">
              <td>宾馆名称</td>
              <td>宾馆等级</td>
              <td>宾馆地址</td>
              <td>联系电话</td>
            ...
            <tr>
              <td v-show="hotelList.length == ''" colspan="7" class="noinfo">没有找到匹配的记录</td>
            </tr>
            <tr v-for="(item, index) in hotelList" :key="index">
              <td style="width:12%">{{item.hotelName}}</td>
              ...
              <td style="width:10%">
                <i class="el-icon-edit basis_edit" @click="change(item.hotelId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.hotelId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        <div class="basis_hotel_content_right_content_page">
          <div class="basis_hotel_content_right_content_page_left">
            <el-pagination
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="宾馆名称" prop="name">
                    <el-input v-model="ruleForm.name"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="宾馆等级" prop="region">
                    <el-select v-model="ruleForm.region" placeholder="请选择宾馆等级">
              ...
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="宾馆地址" prop="address">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                ...
                <el-col :span="12">
                  <el-form-item label="宾馆等级" prop="region">
                    <el-select v-model="ruleFormChange.region" placeholder="请选择宾馆等级">
                      <el-option label="未评级" value="0"></el-option>
                      <el-option label="A" value="1"></el-option>
                      <el-option label="AA" value="2"></el-option>
                      <el-option label="AAA" value="3"></el-option>
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/setPtBaseHotel"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入宾馆名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入宾馆地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择宾馆等级", trigger: "change" }
        ],
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelPtBaseHotelVO";
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getHotelList() {
      ...
      let res = await this.$http.get(
        `/base/listPtBaseHotel?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addHotelList() {
      let res = await this.$http.post(
        `/base/savePtBaseHotel`,
        qs.stringify({
          hotelName: this.ruleForm.name,
    ...
    // 获取编辑宾馆列表
    async getEditHotelList(id) {
      console.log(id);
      let res = await this.$http.get(`/base/getPtBaseHotel?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editHotelList = res.data.data;
        this.ruleFormChange.name = this.editHotelList.hotelName;
        ...
        this.ruleFormChange.introduce = this.editHotelList.synopsis;
        this.ruleFormChange.imgUrl = this.editHotelList.flagPicUrl;
        console.log(this.editHotelList);
      }
    },
    // 编辑宾馆列表
    async editHotelLists() {
      console.log(this.editHotelList);
      this.editHotelList.hotelName = this.ruleFormChange.name;
      ...
      this.editHotelList.synopsis = this.ruleFormChange.introduce;
      this.editHotelList.flagPicUrl = this.ruleFormChange.imgUrl;
      let res = await this.$http.post(
        `/base/updatePtBaseHotel`,
    ...
    async delHotelList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deletePtBaseHotel?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getHotelList();
        this.$message({
...
.basis_hotel_content_right_content {
  width: 100%;
  height: 100%;
  padding: 10px;
  box-sizing: border-box;
}
.basis_hotel_content_right_content_title {
  display: flex;
  align-items: center;
}
.basis_hotel_content_right_content_title input {
  width: 180px;
  height: 32px;
  padding-left: 10px;
  border: 1px solid #ddd;
}
.basis_hotel_content_right_content_title button {
  padding: 3px 15px;
  margin: 0 10px;
  /* height: 20px; */
  text-align: center;
...
.basis_hotel_content_right_content_table {
  width: 100%;
}
.basis_hotel_content_right_content_table table {
  width: 95%;
  margin-top: 20px;
  font-size: 13px;
  border-collapse: collapse;
}
.basis_hotel_content_right_content_table_title {
  background-color: #495e66;
  font-size: 13px;
  font-weight: bold;
  color: #fff;
}
.basis_hotel_content_right_content_table_title > td {
}
.basis_hotel_content_right_content_table_title > td:nth-of-type(1) {
  border-top-left-radius: 10px;
  border-top: none;
  border-left: none;
}
.basis_hotel_content_right_content_table table tr td {
