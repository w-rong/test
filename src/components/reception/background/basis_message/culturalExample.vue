<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入文化产业示范（试验）园区和示范基地名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">文化产业示范（试验）园区和示范基地名称</td>
              <td style="width:20%">文化产业示范（试验）园区和示范基地地址</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              <td>{{item.culturalExampleName}}</td>
              <td>{{item.address}}</td>
              <td>{{item.tel}}</td>
              <td>{{item.lng}}</td>
              <td>{{item.lat}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.culturalExampleId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.culturalExampleId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="14">
                  <el-form-item label="文化产业示范（试验）园区和示范基地名称" label-width="300px" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="14">
                  <el-form-item label="文化产业示范（试验）园区和示范基地地址" label-width="300px" prop="address">
              ...
              <el-col :span="22">
                <el-form-item label-width="300px" label="文化产业示范（试验）园区和示范基地介绍">
        ...
        <el-dialog title="修改数据" :visible.sync="isedited" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="文化产业示范（试验）园区和示范基地名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="文化产业示范（试验）园区和示范基地地址" prop="address">
              ...
              <el-col :span="22">
                <el-form-item label="文化产业示范（试验）园区和示范基地介绍">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/setBaseCulturalExample"
        multiple
        name="uploadFile"
    ...
    return {
      ...
      rules: {
        name: [
          {
            required: true,
            message: "请输入文化产业示范（试验）园区和示范基地名称",
            trigger: "blur"
          }
        ],
        address: [
          {
            required: true,
            message: "请输入文化产业示范（试验）园区和示范基地地址",
            trigger: "blur"
          }
        ],
        region: [
          {
            required: true,
            message: "请选择文化产业示范（试验）园区和示范基地等级",
            trigger: "change"
          }
        ],
      ...
      rulesMax: {
        ...
        maxVolume: [
          {
            required: true,
            message: "请输入文化产业示范（试验）园区和示范基地最大承载量",
            trigger: "blur"
          }
        ]
      },
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelBaseCulturalExample";
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    // 获取文化产业示范（试验）园区和示范基地列表
    async getScenicList() {
      if (this.unitName != "") {
        this.currentPage = 1;
      }
      let res = await this.$http.get(
        `/base/listBaseCulturalExample?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    // 添加文化产业示范（试验）园区和示范基地列表
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseCulturalExample`,
        qs.stringify({
          culturalExampleName: this.ruleForm.name,
    ...
    // 获取编辑文化产业示范（试验）园区和示范基地列表
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseCulturalExample?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.culturalExampleName;
    ...
    // 编辑文化产业示范（试验）园区和示范基地列表
    async editScenicLists() {
      console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseCulturalExample`,
        qs.stringify({
          culturalExampleId: this.editScenicList.culturalExampleId,
          culturalExampleName: this.ruleFormChange.name,
    ...
    // 删除文化产业示范（试验）园区和示范基地列表
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(
        `/base/deleteBaseCulturalExample?id=${id}`
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
