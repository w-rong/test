<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入产业信息名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">产业信息名称</td>
              <td style="width:20%">产业信息地址</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              ...
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.industryId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.industryId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="产业信息名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                <el-form-item label="产业信息地址" prop="address">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseIndustry"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入产业信息名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入产业信息地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择产业信息等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择产业信息", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入产业信息最大承载量", trigger: "blur" }
        ]
      },
      token: {}
    };
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseIndustry'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseIndustry?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseIndustry?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.industryName;
        this.ruleFormChange.address = this.editScenicList.address;
        this.ruleFormChange.leaderPhone = this.editScenicList.fixedPhone;
        this.ruleFormChange.leader = this.editScenicList.leader;
        this.ruleFormChange.introduce = this.editScenicList.synopsis;
        this.ruleFormChange.imgurl = this.editScenicList.industryPic;
        // console.log(this.editScenicList);
        // console.log(this.ruleFormChange.leader);
        // console.log(this.editScenicList.leader);
      }
    },
    // 编辑产业信息列表
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseIndustry`,
        qs.stringify({
          industryId:this.editScenicList.industryId,
          industryName: this.ruleFormChange.name,
          address: this.ruleFormChange.address,
          leader:this.ruleFormChange.leader,
          fixedPhone: this.ruleFormChange.leaderPhone,
          synopsis: this.ruleFormChange.introduce,
          industryPic: this.ruleFormChange.imgurl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseIndustry?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
