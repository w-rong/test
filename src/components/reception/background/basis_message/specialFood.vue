<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入特色菜名名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">特色菜名名称</td>
              <td style="width:20%">餐馆名称</td>
              <td style="width:10%">辣度</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              ...
              <td>{{item.spicy}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.specialFoodId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.specialFoodId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="特色菜名名称" prop="name">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseSpecialFood"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入特色菜名名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入特色菜名地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择特色菜名等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择特色菜名", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入特色菜名最大承载量", trigger: "blur" }
        ]
      },
      token: {}
    };
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseSpecialFood'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseSpecialFood?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseSpecialFood`,
        qs.stringify({
          foodName: this.ruleForm.name,
          restaurantName: this.ruleForm.address,
          spicy: this.ruleForm.lat,
          foodSynopsis: this.ruleForm.introduce,
          foodPic: this.ruleForm.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseSpecialFood?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.foodName;
        this.ruleFormChange.address = this.editScenicList.restaurantName;
        this.ruleFormChange.lat = this.editScenicList.spicy;
        this.ruleFormChange.introduce = this.editScenicList.foodSynopsis;
        this.ruleFormChange.imgUrl = this.editScenicList.foodPic;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseSpecialFood`,
        qs.stringify({
          specialFoodId:this.editScenicList.specialFoodId,
          foodName: this.ruleFormChange.name,
          restaurantName: this.ruleFormChange.address,
          spicy: this.ruleFormChange.lat,
          foodSynopsis: this.ruleFormChange.introduce,
          foodPic: this.ruleFormChange.imgUrl,
        })
      );
      console.log(res);
      if (res.data.msg == "success") {
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseSpecialFood?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
