<template>
  <div class="basis_shopping_all_content">
    <Navbar></Navbar>
    <div class="basis_shopping_content">
      <div class="basis_shopping_content_right_content">
        <div class="basis_shopping_content_right_content_title">
          <input type="text" placeholder="请输入购物场所名称" v-model="unitName" />
        ...
        <div class="basis_shopping_content_right_content_table">
          <table>
            <tr class="basis_shopping_content_right_content_table_title">
              <td>购物场所名称</td>
              <td>购物场所地址</td>
              <td>联系电话</td>
              <td>经度</td>
            ...
            <tr>
              <td v-show="shoppingList.length == ''" colspan="7" class="noinfo">没有找到匹配的记录</td>
            </tr>
            <tr v-for="(item, index) in shoppingList" :key="index">
              <td style="width:12%">{{item.shoppingName}}</td>
              <td style="width:18%">{{item.address}}</td>
              <td style="width:12%">{{item.tel}}</td>
              <td style="width:10%">{{item.lng}}</td>
              <td style="width:10%">{{item.lat}}</td>
              <td style="width:10%">
                <i class="el-icon-edit basis_edit" @click="change(item.shoppingId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.shoppingId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        <div class="basis_shopping_content_right_content_page">
          <div class="basis_shopping_content_right_content_page_left">
            <el-pagination
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="购物场所名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="购物场所地址" prop="address">
      ...
      <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
        <el-upload
          class="upload-demo"
          action="http://47.96.94.56:8082/excel/setPtBaseShopping"
          multiple
          name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: "请输入购物场所名称", trigger: "blur" }
        ],
        address: [
          { required: true, message: "请输入购物场所地址", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelPtBaseShoppingVO";
      // let res = await this.$http.get(`/excel/getExcelPtBaseShoppingVO`);
      // console.log(res);
    },
    beforeUpload(file) {
      // console.log(file);
    ...
    async getShoppingList() {
      ...
      let res = await this.$http.get(
        `/base/listPtBaseShopping?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addShoppingList() {
      let res = await this.$http.post(
        `/base/savePtBaseShopping`,
        qs.stringify({
          shoppingName: this.ruleForm.name,
          address: this.ruleForm.address,
          lat: this.ruleForm.lat,
          lng: this.ruleForm.lng,
          tel: this.ruleForm.tel,
          specialIntroduce: this.ruleForm.introduce,
    ...
    // 获取编辑购物场所列表
    async getEditShoppingList(id) {
      console.log(id);
      let res = await this.$http.get(`/base/getPtBaseShopping?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editShoppingList = res.data.data;
        this.ruleFormChange.name = this.editShoppingList.shoppingName;
        this.ruleFormChange.address = this.editShoppingList.address;
        this.ruleFormChange.lng = this.editShoppingList.lng;
        this.ruleFormChange.lat = this.editShoppingList.lat;
        this.ruleFormChange.tel = this.editShoppingList.tel;
        this.ruleFormChange.introduce = this.editShoppingList.specialIntroduce;
        this.ruleFormChange.imgUrl = this.editShoppingList.flagPicUrl;
        // console.log(this.editShoppingList);
      }
    },
    // 编辑购物场所列表
    async editShoppingLists() {
      console.log(this.editShoppingList);
      this.editShoppingList.shoppingName = this.ruleFormChange.name;
      this.editShoppingList.address = this.ruleFormChange.address;
      this.editShoppingList.lng = this.ruleFormChange.lng;
      this.editShoppingList.lat = this.ruleFormChange.lat;
      this.editShoppingList.tel = this.ruleFormChange.tel;
      this.editShoppingList.specialIntroduce = this.ruleFormChange.introduce;
      this.editShoppingList.flagPicUrl = this.ruleFormChange.imgUrl;
      let res = await this.$http.post(
        `/base/updatePtBaseShopping`,
        qs.stringify(this.editShoppingList)
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.isedited = false;
    ...
    // 删除购物场所列表
    async delShoppingList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deletePtBaseShopping?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getShoppingList();
        this.$message({
    ...
    change(id) {
      ...
      this.isedited = true;
      this.getEditShoppingList(id);
    },
    // 编辑
    submitFormChange(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.editShoppingLists();
        } else {
          console.log("error submit!!");
          return false;
        }
...
<style>
.basis_shopping_all_content {
  width: 100%;
  height: 100%;
  background-color: #f4f4f4;
}
.basis_shopping_content {
...
.basis_shopping_content_right_content {
  width: 100%;
  height: 100%;
  padding: 10px;
  box-sizing: border-box;
}
.basis_shopping_content_right_content_title {
  display: flex;
  align-items: center;
}
.basis_shopping_content_right_content_title input {
  width: 180px;
  height: 32px;
  padding-left: 10px;
  border: 1px solid #ddd;
}
.basis_shopping_content_right_content_title button {
  padding: 3px 15px;
  margin: 0 10px;
  /* height: 20px; */
  text-align: center;
...
.basis_shopping_content_right_content_table {
  width: 100%;
}
.basis_shopping_content_right_content_table table {
  width: 95%;
  margin-top: 20px;
  font-size: 13px;
  border-collapse: collapse;
}
.basis_shopping_content_right_content_table_title {
  background-color: #495e66;
  font-size: 13px;
  font-weight: bold;
  color: #fff;
}
.basis_shopping_content_right_content_table_title > td {
}
.basis_shopping_content_right_content_table_title > td:nth-of-type(1) {
  border-top-left-radius: 10px;
  border-top: none;
  border-left: none;
}
.basis_shopping_content_right_content_table table tr td {
