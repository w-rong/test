<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入文化站名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">文化站名称</td>
              <td style="width:20%">文化站地址</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              <td>{{item.culturalStationName}}</td>
              <td>{{item.address}}</td>
              <td>{{item.tel}}</td>
              <td>{{item.lng}}</td>
              <td>{{item.lat}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.culturalStationId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.culturalStationId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="文化站名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                <el-form-item label="文化站地址" prop="address">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseCulturalStation"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入文化站名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入文化站地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择文化站等级", trigger: "change" }
        ],
      ...
      rulesMax: {
        name: [{ required: true, message: "请选择文化站", trigger: "blur" }],
        maxVolume: [
          { required: true, message: "请输入文化站最大承载量", trigger: "blur" }
        ]
      },
      token: {}
    };
  ...
  methods: {
    ...
    // 导出
    async derive() {
      window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseCulturalStation'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseCulturalStation?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseCulturalStation`,
        qs.stringify({
          culturalStationName: this.ruleForm.name,
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseCulturalStation?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.culturalStationName;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseCulturalStation`,
        qs.stringify({
          culturalStationId:this.editScenicList.culturalStationId,
          culturalStationName: this.ruleFormChange.name,
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseCulturalStation?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
