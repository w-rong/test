<template>
  <div class="basis_scenic_all_content">
    ...
    <div class="basis_scenic_content">
      ...
      <div class="basis_scenic_content_right_content">
        <div class="basis_scenic_content_right_content_title">
          <input type="text" placeholder="请输入文物保护管理机构名称" v-model="unitName" />
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            <tr class="basis_scenic_content_right_content_table_title">
              <td style="width:12%">文物保护管理机构名称</td>
              <td style="width:20%">文物保护管理机构地址</td>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              <td>{{item.culturalProtectScienceName}}</td>
              <td>{{item.address}}</td>
              <td>{{item.tel}}</td>
              <td>{{item.lng}}</td>
              <td>{{item.lat}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.culturalProtectScienceId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.culturalProtectScienceId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="文物保护管理机构名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="文物保护管理机构地址" prop="address">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseCulturalProtectScience"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rulesMax: {
        name: [
          { required: true, message: "请选择文物保护管理机构", trigger: "blur" }
        ],
        maxVolume: [
          {
            required: true,
            message: "请输入文物保护管理机构最大承载量",
            trigger: "blur"
          }
        ]
      },
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelBaseCulturalProtectScience";
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseCulturalProtectScience?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseCulturalProtectScience`,
        qs.stringify({
          culturalProtectScienceName: this.ruleForm.name,
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(
        `/base/getBaseCulturalProtectScience?id=${id}`
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.culturalProtectScienceName;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseCulturalProtectScience`,
        qs.stringify({
          culturalProtectScienceId: this.editScenicList
            .culturalProtectScienceId,
          culturalProtectScienceName: this.ruleFormChange.name,
    ...
    async delScenicList(id) {
      ...
      let res = await this.$http.delete(
        `/base/deleteBaseCulturalProtectScience?id=${id}`
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
