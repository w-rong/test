<template>
  <div class="basis_travel_all_content">
    <Navbar></Navbar>
    <div class="basis_travel_content">
      <div class="basis_travel_content_right_content">
        <div class="basis_travel_content_right_content_title">
          <input type="text" placeholder="请输入旅行社名称" v-model="unitName" />
        ...
        <div class="basis_travel_content_right_content_table">
          <table>
            <tr class="basis_travel_content_right_content_table_title">
              <td>旅行社名称</td>
              <td>旅行社地址</td>
              <td>联系电话</td>
              <td>经度</td>
            ...
            <tr>
              <td v-show="travelList.length == ''" colspan="7" class="noinfo">没有找到匹配的记录</td>
            </tr>
            <tr v-for="(item, index) in travelList" :key="index">
              <td style="width:12%">{{item.travelName}}</td>
              <td style="width:18%">{{item.address}}</td>
              <td style="width:12%">{{item.tel}}</td>
              <td style="width:10%">{{item.lng}}</td>
              <td style="width:10%">{{item.lat}}</td>
              <td style="width:10%">
                <i class="el-icon-edit basis_edit" @click="change(item.travelId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.travelId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        <div class="basis_travel_content_right_content_page">
          <div class="basis_travel_content_right_content_page_left">
            <el-pagination
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              ...
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="旅行社地址" prop="address">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/setPtBaseTravel"
        multiple
        name="uploadFile"
  ...
  data() {
    return {
      ...
      rules: {
        ...
        address: [
          { required: true, message: "请输入旅行社地址", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelPtBaseTravelVO";
      // let res = await this.$http.get(`/excel/getExcelPtBaseTravelVO`);
      // console.log(res);
    },
    beforeUpload(file) {
      // console.log(file);
    ...
    async addTravelList() {
      let res = await this.$http.post(
        `/base/savePtBaseTravel`,
        qs.stringify({
          travelName: this.ruleForm.name,
    ...
    // 获取编辑旅行社列表
    async getEditTravelList(id) {
      console.log(id);
      let res = await this.$http.get(`/base/getPtBaseTravel?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editTravelList = res.data.data;
        this.ruleFormChange.name = this.editTravelList.travelName;
        this.ruleFormChange.address = this.editTravelList.address;
        this.ruleFormChange.lng = this.editTravelList.lng;
        this.ruleFormChange.lat = this.editTravelList.lat;
        this.ruleFormChange.tel = this.editTravelList.tel;
        this.ruleFormChange.introduce = this.editTravelList.synopsis;
        // console.log(this.editTravelList);
      }
    },
    // 编辑旅行社列表
    async editTravelLists() {
      console.log(this.editTravelList);
      this.editTravelList.travelName = this.ruleFormChange.name;
      this.editTravelList.address = this.ruleFormChange.address;
      this.editTravelList.lng = this.ruleFormChange.lng;
      this.editTravelList.lat = this.ruleFormChange.lat;
      this.editTravelList.tel = this.ruleFormChange.tel;
      this.editTravelList.synopsis = this.ruleFormChange.introduce;
      let res = await this.$http.post(
        `/base/updatePtBaseTravel`,
        qs.stringify(this.editTravelList)
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.isedited = false;
    ...
    // 删除旅行社列表
    async delTravelList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deletePtBaseTravel?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getTravelList();
        this.$message({
    ...
    change(id) {
      ...
      this.isedited = true;
      this.getEditTravelList(id);
    },
    // 编辑
    submitFormChange(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.editTravelLists();
        } else {
          console.log("error submit!!");
          return false;
        }
...
<style>
.basis_travel_all_content {
  width: 100%;
  height: 100%;
  background-color: #f4f4f4;
}
.basis_travel_content {
...
.basis_travel_content_right_content {
  width: 100%;
  height: 100%;
  padding: 10px;
  box-sizing: border-box;
}
.basis_travel_content_right_content_title {
  display: flex;
  align-items: center;
}
.basis_travel_content_right_content_title input {
  width: 180px;
  height: 32px;
  padding-left: 10px;
  border: 1px solid #ddd;
}
.basis_travel_content_right_content_title button {
  padding: 3px 15px;
  margin: 0 10px;
  /* height: 20px; */
  text-align: center;
...
.basis_travel_content_right_content_table {
  width: 100%;
}
.basis_travel_content_right_content_table table {
  width: 95%;
  margin-top: 20px;
  font-size: 13px;
  border-collapse: collapse;
}
.basis_travel_content_right_content_table_title {
  background-color: #495e66;
  font-size: 13px;
  font-weight: bold;
  color: #fff;
}
.basis_travel_content_right_content_table_title > td {
}
.basis_travel_content_right_content_table_title > td:nth-of-type(1) {
  border-top-left-radius: 10px;
  border-top: none;
  border-left: none;
}
.basis_travel_content_right_content_table table tr td {
