<template>
  <div class="basis_library_all_content">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseLibrary"
        multiple
        name="uploadFile"
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelBaseLibrary";
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getEditLibraryList(id) {
      ...
      if (res.data.msg == "success") {
        ...
        this.ruleFormChange.introduce = this.editLibraryList.synopsis;
        this.ruleFormChange.imgUrl = this.editLibraryList.pic;
    ...
    async editLibraryLists() {
      ...
      this.editLibraryList.synopsis = this.ruleFormChange.introduce;
      this.editLibraryList.pic = this.ruleFormChange.imgUrl;
      let res = await this.$http.post(
