<template>
  <div class="basis_scenic_all_content">
        ...
        <div class="basis_scenic_content_right_content_table">
          <table>
            ...
            <tr v-for="(item, index) in scenicList" :key="index">
              <td>{{item.culturalProtectName}}</td>
              <td>{{item.address}}</td>
              <td>{{item.tel}}</td>
              <td>{{item.lng}}</td>
              <td>{{item.lat}}</td>
              <td>
                <i class="el-icon-edit basis_edit" @click="change(item.culturalProtectId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.culturalProtectId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseCulturalProtect"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: "请输入文物保护管理机构名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入文物保护管理机构地址", trigger: "blur" }
        ],
        region: [
          { required: true, message: "请选择文物保护管理机构等级", trigger: "change" }
        ],
  ...
  methods: {
    ...
    // 导出
    async derive() {
       window.location.href = 'http://47.96.94.56:8082/excel/getExcelBaseCulturalProtect'
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getScenicList() {
      ...
      let res = await this.$http.get(
        `/base/listBaseCulturalProtect?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
    ...
    async addScenicList() {
      let res = await this.$http.post(
        `/base/saveBaseCulturalProtect`,
        qs.stringify({
          culturalProtectName: this.ruleForm.name,
    ...
    async getEditScenicList(id) {
      let res = await this.$http.get(`/base/getBaseCulturalProtect?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editScenicList = res.data.data;
        this.ruleFormChange.name = this.editScenicList.culturalProtectName;
    ...
    async editScenicLists() {
      // console.log(this.editScenicList);
      let res = await this.$http.post(
        `/base/updateBaseCulturalProtect`,
        qs.stringify({
          culturalProtectId:this.editScenicList.culturalProtectId,
          culturalProtectName: this.ruleFormChange.name,
    ...
    async delScenicList(id) {
      console.log(id);
      let res = await this.$http.delete(`/base/deleteBaseCulturalProtect?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.getScenicList();
        this.$message({
