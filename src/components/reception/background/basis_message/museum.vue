<template>
  <div class="basis_museum_all_content">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseMuseum"
        multiple
        name="uploadFile"
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelBaseMuseum";
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getEditMuseumList(id) {
      ...
      if (res.data.msg == "success") {
        ...
        this.ruleFormChange.introduce = this.editMuseumList.synopsis;
        this.ruleFormChange.imgUrl = this.editMuseumList.pic;
    ...
    async editMuseumLists() {
      ...
      this.editMuseumList.synopsis = this.ruleFormChange.introduce;
      this.editMuseumList.pic = this.ruleFormChange.imgUrl;
      let res = await this.$http.post(
