<template>
  <div class="basis_culture_all_content">
    <Navbar></Navbar>
    <div class="basis_culture_content">
      <div class="basis_culture_content_right_content">
        <div class="basis_culture_content_right_content_title">
          <input type="text" placeholder="请输入文化馆名称" v-model="unitName" />
        ...
        <div class="basis_culture_content_right_content_table">
          <table>
            <tr class="basis_culture_content_right_content_table_title">
              <td>文化馆名称</td>
              <td>文化馆地址</td>
              <td>联系电话</td>
              <td>经度</td>
            ...
            <tr>
              <td v-show="cultureList.length == ''" colspan="7" class="noinfo">没有找到匹配的记录</td>
            </tr>
            <tr v-for="(item, index) in cultureList" :key="index">
              <td style="width:12%">{{item.culturalCenterName}}</td>
              <td style="width:18%">{{item.address}}</td>
              <td style="width:12%">{{item.tel}}</td>
              <td style="width:10%">{{item.lng}}</td>
              <td style="width:10%">{{item.lat}}</td>
              <td style="width:10%">
                <i class="el-icon-edit basis_edit" @click="change(item.culturalCenterId)">编辑</i>
                <i class="el-icon-delete basis_del" @click="del(item.culturalCenterId)">删除</i>
              </td>
            </tr>
          </table>
        </div>
        <div class="basis_culture_content_right_content_page">
          <div class="basis_culture_content_right_content_page_left">
            <el-pagination
        ...
        <el-dialog title="新增数据" :visible.sync="isadded" width="60%" :modal-append-to-body="true">
          ...
          >
            <el-row :span="24">
              <el-col :span="22">
                <el-col :span="12">
                  <el-form-item label="文化馆名称" prop="name">
              ...
              <el-col :span="22">
                <el-col :span="12">
                <el-form-item label="文化馆地址" prop="address">
...
<el-col :span="10" style="margin-left:10px;margin-top:5px">
    ...
    <el-dialog title="导入文件" :visible.sync="isLead" width="30%">
      <el-upload
        class="upload-demo"
        action="http://47.96.94.56:8082/excel/importBaseCulturalCenter"
        multiple
        name="uploadFile"
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: "请输入文化馆名称", trigger: "blur" }
        ],
        address: [
          { required: true, message: "请输入文化馆地址", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    async derive() {
      window.location.href =
        "http://47.96.94.56:8082/excel/getExcelBaseCulturalCenter";
    },
    beforeUpload(file) {
      // console.log(file);
      var testmsg = file.name.substring(file.name.lastIndexOf(".") + 1);
    ...
    async getCultureList() {
      let res = await this.$http.get(
        `/base/listBaseCulturalCenter?pagNumber=${this.currentPage}&pagSize=${this.pageSize}&unitName=${this.unitName}`
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.cultureList = res.data.data.list;
        this.allAccount = res.data.data.total;
      }
    },
    // 添加文化馆列表
    async addCultureList() {
      let res = await this.$http.post(
        `/base/saveBaseCulturalCenter`,
        qs.stringify({
          culturalCenterName: this.ruleForm.name,
    ...
    // 获取编辑文化馆列表
    async getEditCultureList(id) {
      console.log(id);
      let res = await this.$http.get(`/base/getBaseCulturalCenter?id=${id}`);
      console.log(res);
      if (res.data.msg == "success") {
        this.editCultureList = res.data.data;
        this.ruleFormChange.name = this.editCultureList.culturalCenterName;
        this.ruleFormChange.address = this.editCultureList.address;
        this.ruleFormChange.lng = this.editCultureList.lng;
        this.ruleFormChange.lat = this.editCultureList.lat;
        this.ruleFormChange.tel = this.editCultureList.tel;
        this.ruleFormChange.introduce = this.editCultureList.synopsis;
        this.ruleFormChange.imgUrl = this.editCultureList.pic;
        // console.log(this.editCultureList);
      }
    },
    // 编辑文化馆列表
    async editCultureLists() {
      console.log(this.editCultureList);
      this.editCultureList.culturalCenterName = this.ruleFormChange.name;
      this.editCultureList.address = this.ruleFormChange.address;
      this.editCultureList.lng = this.ruleFormChange.lng;
      this.editCultureList.lat = this.ruleFormChange.lat;
      this.editCultureList.tel = this.ruleFormChange.tel;
      this.editCultureList.synopsis = this.ruleFormChange.introduce;
      this.editCultureList.pic = this.ruleFormChange.imgUrl;
      let res = await this.$http.post(
        `/base/updateBaseCulturalCenter`,
        qs.stringify(this.editCultureList)
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.isedited = false;
    ...
    // 删除文化馆列表
    async delCultureList(id) {
      console.log(id);
      let res = await this.$http.delete(
        `/base/deleteBaseCulturalCenter?id=${id}`
      );
      console.log(res);
      if (res.data.msg == "success") {
        this.getCultureList();
    ...
    change(id) {
      ...
      this.isedited = true;
      this.getEditCultureList(id);
    },
    // 编辑
    submitFormChange(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.editCultureLists();
        } else {
          console.log("error submit!!");
          return false;
        }
...
<style>
.basis_culture_all_content {
  width: 100%;
  height: 100%;
  background-color: #f4f4f4;
}
.basis_culture_content {
...
.basis_culture_content_right_content {
  width: 100%;
  height: 100%;
  padding: 10px;
  box-sizing: border-box;
}
.basis_culture_content_right_content_title {
  display: flex;
  align-items: center;
}
.basis_culture_content_right_content_title input {
  width: 180px;
  height: 32px;
  padding-left: 10px;
  border: 1px solid #ddd;
}
.basis_culture_content_right_content_title button {
  padding: 3px 15px;
  margin: 0 10px;
  /* height: 20px; */
  text-align: center;
...
.basis_culture_content_right_content_table {
  width: 100%;
}
.basis_culture_content_right_content_table table {
  width: 95%;
  margin-top: 20px;
  font-size: 13px;
  border-collapse: collapse;
}
.basis_culture_content_right_content_table_title {
  background-color: #495e66;
  font-size: 13px;
  font-weight: bold;
  color: #fff;
}
.basis_culture_content_right_content_table_title > td {
}
.basis_culture_content_right_content_table_title > td:nth-of-type(1) {
  border-top-left-radius: 10px;
  border-top: none;
  border-left: none;
}
.basis_culture_content_right_content_table table tr td {
